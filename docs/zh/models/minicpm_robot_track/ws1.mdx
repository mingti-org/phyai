...
本指南使用PhyAI Engine运行MiniCPM-RobotTrack。模型输入32帧RGB
图像和一条文本指令，输出8个`[x, y, yaw]` waypoint。示例只包含
本地推理，不启动网络服务。
...
端到端示例会初始化32帧窗口，再提交一帧增量输入，并检查输出是
有限值且shape为`(1, 8, 3)`。
...
stream的第一个请求必须包含32帧。`frame_index`是窗口最后一帧的序号。
后续请求每次提交一帧，并使用连续序号。
...
已经有coarse/fine视觉token时，使用`benchmark.py`只测固定MiniCPM
policy：
